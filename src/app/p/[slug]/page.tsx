    ...
    <div style={{ background: theme.bg, minHeight: '100vh', fontFamily: 'DM Sans, sans-serif' }}>
      <style>{`
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .faq-item { border-bottom: 1px solid ${theme.border}; }
        .faq-item summary { padding: 16px 20px; display: flex; justify-content: space-between; align-items: center; font-size: 14px; font-weight: 600; color: ${theme.text}; user-select: none; }
        .faq-item .faq-answer { padding: 0 20px 16px; font-size: 14px; color: ${theme.muted}; line-height: 1.75; }
      `}</style>

      <div style={{ maxWidth: '720px', margin: '0 auto', padding: '0 16px' }}>
        {blocks.map((block: any) => (
          <BlockPublic key={block.id} block={block} theme={theme} />
        ))}
      </div>
    </div>
  )
...
function BlockPublic({ block, theme }: { block: any; theme: any }) {
    ...
    case 'rich':
      ...
      return (
        <div style={{ padding: '40px 0' }}>
          {block.title && (
            <h2 style={{ fontSize: 'clamp(24px,5vw,42px)', fontWeight: block.titleBold ? '700' : '300', fontFamily: block.fontFamily || 'Syne, sans-serif', color: block.titleColor || theme.text, marginBottom: '12px', lineHeight: 1.15, letterSpacing: '-0.5px' }}>
              {rt(block.title)}
            </h2>
          )}
          ...
          {block.sections?.map((s: any) => (
            <div key={s.id} style={{ background: theme.surface, border: `1px solid ${theme.border}`, borderRadius: '12px', padding: '16px 20px', marginBottom: '10px', display: 'flex', gap: '14px', alignItems: 'flex-start' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px', marginTop: '16px' }}>
              {block.beforeImage && (
                <div>
                  ...
                  <img src={block.beforeImage} alt="Antes" style={{ width: '100%', borderRadius: '10px', border: `1px solid ${theme.border}` }}/>
              ...
              {block.afterImage && (
                <div>
                  ...
                  <img src={block.afterImage} alt="Depois" style={{ width: '100%', borderRadius: '10px', border: `1px solid ${theme.accent}` }}/>
            ...
            <div style={{ background: theme.surface, border: `1px solid ${theme.border}`, borderRadius: '14px', overflow: 'hidden', marginTop: '12px' }}>
              {block.faqItems.map((faq: any, i: number) => (
                <details key={i} className="faq-item">
                  <summary>
                    <span>{faq.q}</span>
                    <span style={{ fontSize: '18px', fontWeight: '300', color: theme.muted, flexShrink: 0, marginLeft: '12px' }}>+</span>
                  </summary>
          ...
          {/* Imagem simples */}
          {block.imageUrl && !block.beforeImage && (
            <img src={block.imageUrl} alt={block.imageAlt || ''} style={{ width: '100%', borderRadius: '12px', marginTop: '12px' }}/>
          )}
        </div>
      )
    ...
    case 'insight':
            ...
            {['00', '10', '00'].map((v, i) => (
              <div key={i} style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                <div style={{ background: theme.surface, border: `1px solid ${theme.border}`, borderRadius: '10px', padding: '12px 20px', textAlign: 'center' }}>
                  <div style={{ fontSize: '32px', fontWeight: '700', fontFamily: 'monospace', color: theme.accent }}>{v}</div>
                  <div style={{ fontSize: '10px', color: theme.muted }}>{['HORAS','MIN','SEG'][i]}</div>
                </div>
      ...
      return (
        <div style={{ padding: '16px 0' }}>
          <div style={{
            background: block.alertType === 'warning' ? 'rgba(251,191,36,0.1)' : block.alertType === 'success' ? 'rgba(34,197,94,0.1)' : block.alertType === 'error' ? 'rgba(248,113,113,0.1)' : 'rgba(37,99,255,0.1)',
            border: `1px solid ${block.alertType === 'warning' ? 'rgba(251,191,36,0.4)' : block.alertType === 'success' ? 'rgba(34,197,94,0.4)' : block.alertType === 'error' ? 'rgba(248,113,113,0.4)' : 'rgba(37,99,255,0.4)'}`,
            borderLeft: `4px solid ${block.alertType === 'warning' ? '#fbbf24' : block.alertType === 'success' ? '#22c55e' : block.alertType === 'error' ? '#f87171' : theme.accent}`,
          ...
          <div style={{ background: theme.surface, border: `1px solid ${theme.border}`, borderRadius: '16px', padding: '24px' }}>
            {block.testimonialStars && (
              <div style={{ marginBottom: '12px' }}>
                {'★'.repeat(block.testimonialStars).split('').map((s, i) => <span key={i} style={{ color: '#fbbf24', fontSize: '18px' }}>{s}</span>)}
              </div>
            )}
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
              {block.testimonialPhoto
                ? <img src={block.testimonialPhoto} alt={block.testimonialName} style={{ width: '42px', height: '42px', borderRadius: '50%', objectFit: 'cover' }}/>
    ...
    case 'offer':
      ...
      if (block.plans?.length > 0) return (
        <div style={{ padding: '32px 0', textAlign: 'center' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: `repeat(${block.plans.length}, 1fr)`, gap: '16px' }}>
            {block.plans.map((plan: any, i: number) => (
              <div key={i} style={{ background: theme.surface, border: `1px solid ${plan.highlight ? theme.accent : theme.border}`, borderRadius: '16px', padding: '24px', position: 'relative', boxShadow: plan.highlight ? `0 0 32px ${theme.accent}40` : 'none' }}>
                ...
                {plan.features?.map((f: string, fi: number) => (
                  <div key={fi} style={{ display: 'flex', gap: '8px', alignItems: 'center', marginBottom: '8px', fontSize: '13px', color: theme.text, textAlign: 'left' }}>
                    <span style={{ color: theme.accent2, flexShrink: 0 }}>✓</span>{f}
                  </div>
                ))}
                {plan.buttonText && (
                  <a href={plan.buttonUrl || '#'} style={{ display: 'block', marginTop: '16px', background: plan.highlight ? `linear-gradient(135deg, ${theme.accent}, ${theme.accent}cc)` : 'transparent', border: `1px solid ${plan.highlight ? 'transparent' : theme.border}`, color: plan.highlight ? '#fff' : theme.text, textAlign: 'center', padding: '12px', borderRadius: '10px', fontSize: '14px', fontWeight: '700', textDecoration: 'none', fontFamily: 'Syne, sans-serif', boxShadow: plan.highlight ? `0 0 20px ${theme.accent}50` : 'none' }}>
                    {plan.buttonText}
                  </a>
                )}
              </div>
        ...
        <div style={{ padding: '32px 0', textAlign: 'center' }}>
          ...
          <div style={{ background: theme.surface, border: `1px solid ${theme.border}`, borderRadius: '20px', padding: '28px', display: 'inline-block', minWidth: '260px', textAlign: 'left', boxShadow: `0 0 40px ${theme.accent}20` }}>
            ...
            {block.priceFeatures?.map((f: string, i: number) => (
              <div key={i} style={{ display: 'flex', gap: '10px', alignItems: 'center', marginBottom: '10px', fontSize: '14px', color: theme.text }}>
                <span style={{ color: theme.accent2 }}>✓</span>{f}
              </div>
            ))}
            {block.priceButtonText && (
              <a href={block.priceButtonUrl || '#'} style={{ display: 'block', marginTop: '20px', background: `linear-gradient(135deg, ${theme.accent}, ${theme.accent}cc)`, color: '#fff', textAlign: 'center', padding: '14px', borderRadius: '12px', fontSize: '15px', fontWeight: '700', textDecoration: 'none', fontFamily: 'Syne, sans-serif', boxShadow: `0 0 24px ${theme.accent}50` }}>
                {block.priceButtonText}
              </a>
            )}
          </div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
              {(block.options || []).map((opt: string, i: number) => (
                <div key={i} style={{ display: 'flex', alignItems: 'center', gap: '14px', padding: '14px 18px', border: `1px solid ${theme.border}`, borderRadius: '12px', background: theme.surface, cursor: 'pointer' }}>
                  <div style={{ width: '20px', height: '20px', borderRadius: '50%', border: `2px solid ${theme.muted}`, flexShrink: 0 }}/>
    ...
    case 'capture':
      return (
        <div style={{ padding: '32px 0' }}>
          ...
          {block.subtitle && <p style={{ fontSize: '14px', color: theme.muted, marginBottom: '20px' }}>{block.subtitle}</p>}
          <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
            {block.type === 'capture' && ['Nome completo', 'E-mail', 'WhatsApp'].map((f, i) => (
              <input key={i} placeholder={f} style={{ background: theme.surface, border: `1px solid ${theme.border}`, borderRadius: '12px', padding: '14px 16px', color: theme.text, fontSize: '15px', outline: 'none', width: '100%' }}/>
            ))}
            {block.type === 'field' && block.fieldType === 'height_weight' ? (
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
                <input placeholder={block.heightLabel || 'Altura (cm)'} style={{ background: theme.surface, border: `1px solid ${theme.border}`, borderRadius: '12px', padding: '14px 16px', color: theme.text, fontSize: '15px', outline: 'none' }}/>
                <input placeholder={block.weightLabel || 'Peso (kg)'} style={{ background: theme.surface, border: `1px solid ${theme.border}`, borderRadius: '12px', padding: '14px 16px', color: theme.text, fontSize: '15px', outline: 'none' }}/>
              </div>
            ) : block.type === 'field' && (
              <input placeholder={block.fieldPlaceholder || 'Digite aqui...'} type={block.fieldType === 'email' ? 'email' : block.fieldType === 'phone' ? 'tel' : 'text'} style={{ background: theme.surface, border: `1px solid ${theme.border}`, borderRadius: '12px', padding: '14px 16px', color: theme.text, fontSize: '15px', outline: 'none', width: '100%' }}/>
            )}
            <button style={{ background: `linear-gradient(135deg, ${theme.accent}, ${theme.accent}cc)`, color: '#fff', border: 'none', borderRadius: '12px', padding: '16px', fontSize: '16px', fontWeight: '700', fontFamily: 'Syne, sans-serif', cursor: 'pointer', boxShadow: `0 0 24px ${theme.accent}50` }}>
              {block.buttonText || 'Continuar →'}
            </button>
          </div>
        </div>
